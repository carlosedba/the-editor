...
export default function EditorContentBlockRichSquareCard(props) {
  ...
  const DND_TYPE = DND_EDITOR_SIDEBAR_BLOCK_RICH_SQUARE_CARD
  ...
  function renderItems() {
    return content.map((item, i) => {
      return (
        <div className="editor-content-block-rich-square-card-item" key={i}>
          <div className="editor-content-block-rich-square-card-item__icon" onContextMenu={(event) => show({ event: event, props: item })}>
            <input className="editor-content-block-rich-square-card-item__icon-input" type="file" accept="image/svg+xml" onChange={(event) => handleItemIconChange(event, i)} ref={(el) => fileInputs.current[i] = el}/>
            <div className="editor-content-block-rich-square-card-item__icon-preview" dangerouslySetInnerHTML={{ __html: item.icon }}></div>
          </div>
